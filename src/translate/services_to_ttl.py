...
#patient_df = df[['PBRN_PRACTICE', 'DB_PRACTICE_ID', 'PATIENT_ID', 'TOOTH', 'SURFACE', 'TRAN_DATE', 'ADA_CODE', 'PROVIDER_ID', 'TABLE_NAME']]
    ...
    patient_df = df[['db_practice_id', 'patient_id', 'tooth', 'surface', 'date_completed', 'date_entered', 'ada_code', 'provider_id', 'table_name', 'tooth_data']]
    ...
    try:
        filename = output_p + 'existing_services_' + procedure_type_map[str(procedure_type)] + '.trig'
        err_filename = output_p + 'existing_services_' + procedure_type_map[str(procedure_type)] + '_err.txt'
    ...
    with open(filename, 'w') as f:
        with open(err_filename, 'w') as f_err:
            ...
            for (idx, locationId, pid, tooth_num_in_file, surface, complete_date, enter_date, ada_code, prov_id, tableName, tooth_data) in patient_df.itertuples():
                    ...
                    if len(ada_code) != 5:
                        ...
                        if visit_date_str != 'invalid date':
                            cdt_code_id = str(practiceId) + "_" + str(locationId) + "_" + str(pid) + "_" + str(
                                get_ada_code(ada_code, idx)) + "_" + visit_date_str
                        else:
                            cdt_code_id = str(practiceId) + "_" + str(locationId) + "_" + str(pid) + "_" + str(
                        ...
                        if procedure_date_str != 'invalid date':
                            ...
                            if str(procedure_type) == '9':  ## for surgic tooth extraction
                                restoration_procedure_label = "extraction procedure on patient " + str(pid)
                        ...
                        visit_id = str(practiceId) + "_" + str(locationId) + "_" + str(
                            pid) + "_" + get_visit_id_suffix_with_date_str(visit_date_str, idx)
                        ...
                        # procedure "is part of" previous visit with date:
                        if procedure_date_str != 'invalid date':
                            output(
                                ohd_ttl['relate uri part of previous visit with date'].
                        ...
                        try:
                            ...
                            locationId = int(locationId)
                            visit_id = str(practiceId) + "_" + str(locationId) + "_" + str(pid) + "_" + get_visit_id_suffix_with_date_str(visit_date_str, idx)
                            ...
                            try:
                                ...
                                if str(procedure_type) == '1':  ## for filling
                                    ...
                                    output_err(
                                        "Invalid procedure type: " + str(procedure_type) + " for patient: " + str(
                                            pid) + " for practice: " + str(practiceId))
                                    return
                                ...
                                if continue_flag_filter_with_procedure:
                                    ...
                                    ## refactor to make tooth_num NOT required
                                        ...
                                        for tooth_num in tooth_num_array:
                                            ...
                                            if visit_date_str != 'invalid date':
                                                cdt_code_id = tooth_id + "_" + ada_code + "_" + visit_date_str
                                            else:
                                            ...
                                            if str(procedure_type) == '8' or str(procedure_type) == '11' or str(procedure_type) == '12' \
                                                ...
                                                tooth_str = ohd_ttl['declare prosthetic tooth by prefix'].format(tooth_id=tooth_id,
                                                                                                                 ...
                                                                                                                 practice_id_str=practiceidstring)
                                                if visit_date_str != 'invalid date':
                                                    fixed_partial_denture_id = str(practiceId) + "_" + str(locationId) + "_" + str(pid) + "_" + visit_date_str
                                                else:
                                            ...
                                            if procedure_date_str != 'invalid date':
                                                ...
                                                if str(procedure_type) == '9':  ## for surgic tooth extraction
                                                    restoration_procedure_label = "extraction procedure on " + tooth_label + " on " + procedure_date_str
                                            else:
                                                ...
                                                if str(procedure_type) == '9':  ## for surgic tooth extraction
                                                    restoration_procedure_label = "extraction procedure on " + tooth_label
                                            specific_procedure = label2uri[load_ada_procedure_map[ada_code]].rsplit('/', 1)[-1]
                                            restoration_procedure = ohd_ttl['declare restoration procedure'].format(cdt_code_id=cdt_code_id,
                                                                                                                    tooth_restoration_procedure=specific_procedure,
                                                                                                                    label=restoration_procedure_label,
                                            ...
                                            if str(procedure_type) == '1' or str(procedure_type) == '3' or str(procedure_type) == '4':  ## for filling/inlay/onlay (with surface info)
                                                if pds.notnull(surface) and surface:
                                                    ...
                                                    # existing services translation for finding and evaluatoin:
                                                    finding_label = "dental finding for patient: " + str(
                                                        patient_id) + " of " + ada_code + " on " + visit_date_str
                                                    finding_id = str(tooth_id) + "_" + surface + "_" + ada_code + "_" + visit_date_str
                                                    finding_uri = "dental_finding:" + finding_id
                                                    finding_str = ohd_ttl['declare obo type with label']. \
                                                        format(uri=finding_uri,
                                                               type=
                                                               label2uri['dental procedure finding'].rsplit('/', 1)[-1],
                                                               ## 'dental procedure finding'
                                                               label=finding_label,
                                                               practice_id_str=practiceidstring)
